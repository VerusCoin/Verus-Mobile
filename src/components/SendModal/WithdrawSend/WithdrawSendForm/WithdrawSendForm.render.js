...
export const WithdrawSendFormRender = function () {
  return this.state.loading ? (
    <AnimatedActivityIndicatorBox />
  ) : this.state.selectCurrencyModalParams.open ? (
    ConversionCurrencyListRender.call(this)
  ) : this.state.destinationListParams.open ? (
    DestinationListRender.call(this)
  ) : (
    WithdrawSendFormRenderInputForm.call(this)
  );
};

export const WithdrawSendFormRenderInputForm = function() {
  const { data, coinObj } = this.props.sendModal;
  const { sendAmount, receiveAmount } = this.state;
  const toCurrency = data[SEND_MODAL_TO_CURRENCY_FIELD].destinationCurrencyId;
  const amount = data[SEND_MODAL_AMOUNT_FIELD];
  const price =
    data[SEND_MODAL_TO_CURRENCY_FIELD].price == null ? 0 : data[SEND_MODAL_TO_CURRENCY_FIELD].price;
            ...
            <View style={{ alignItems: "center", justifyContent: "center" }}>
              <View style={Styles.flexRow}>
                ...
                <TouchableOpacity
                  ...
                  onPress={() => this.openDestinationCurrencyOptions()}
                >
                  <TextInput
                    label={"Currency"}
                    editable={false}
              ...
              <View style={Styles.flexRow}>
                ...
                >
                  <TextInput
                    ...
                    label={"Destination"}
                    value={
                      data[SEND_MODAL_DESTINATION_FIELD] == null
                        ? ""
                        : data[SEND_MODAL_DESTINATION_FIELD].displayName
                    }
                    mode="outlined"
                  />
                </TouchableOpacity>
        ...
        <View style={{ ...Styles.wideBlock, paddingTop: 0 }}>
          ...
          >
            <Title style={{ color: Colors.basicButtonColor, fontSize: 16 }}>{`Price per ${
              coinObj == null ? "-" : coinObj.display_ticker
            }: ${toCurrency == null ? "-" : `${price} ${toCurrency}`}`}</Title>
          </Card>
        </View>
        <View style={{ ...Styles.wideBlock, paddingTop: 0 }}>
...
export const ConversionCurrencyListRender = function () {
  return (
    <View style={{ ...Styles.centerContainer, backgroundColor: Colors.secondaryColor }}>
      <List.Item
        title={"Back"}
        description={"Return to withdraw form"}
  ...
  return (
    <View style={{ ...Styles.centerContainer, backgroundColor: Colors.secondaryColor }}>
      <List.Item
        ...
        description={"Return to withdraw form"}
        left={(props) => <List.Icon {...props} icon={"keyboard-backspace"} size={20} />}
        onPress={() => this.selectDestinationOption(null)}
        style={{
          width: "100%",
        }}
      />
      <FlatList
        ...
        renderItem={({ item }) => {
          return (
            <TouchableOpacity onPress={() => this.selectDestinationOption(item.value)}>
              <List.Item
                title={item.title}
                description={item.description}
                left={() =>
        ...
        data={this.state.destinationListParams.options}
      />
    </View>
  );
};